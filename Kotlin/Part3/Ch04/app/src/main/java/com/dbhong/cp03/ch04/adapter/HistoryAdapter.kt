package com.dbhong.cp03.ch04.adapter
...
import com.dbhong.cp03.ch04.databinding.ItemHistoryBinding
...
class HistoryAdapter(val historyDeleteClickedListener : (String) -> Unit, val historyClickedListener : (String) -> Unit) : ListAdapter<History, HistoryAdapter.HistoryItemViewHolder>(diffUtil) {
    ...
    inner class HistoryItemViewHolder(private val binding : ItemHistoryBinding) : RecyclerView.ViewHolder(binding.root){
        ...
        fun bind(historyModel : History) {
            binding.historyKeywordTitleTextView.text = historyModel.keyword

            binding.historyKeywordDeleteButton.setOnClickListener {
                historyDeleteClickedListener(historyModel.keyword.orEmpty())
            }

            binding.historyKeywordTitleTextView.setOnClickListener {
                historyClickedListener(binding.historyKeywordTitleTextView.text.toString())
            }
        }
    }
